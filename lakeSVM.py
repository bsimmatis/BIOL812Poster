...
L06137=pd.ExcelFile('./06 137 NIRS.xls')
L08160=pd.ExcelFile('./08 160 NIRS.xls')
...
#Define logical comparisons for detecting int, str, float
def is_numeric(s):
    try:
        float(s)
        return True
...
#Define a script to identify the absorbances in the 650-700nm range. Half of the column headers are strings (!!!)
#dat_header is a header input, dat is an input file. See the code graveyard for this project for more details
...
L08160_dates=pd.ExcelFile('./08-160 Dates.xlsx')
...
ageData=[ages_L06137, ages_L08160]
datesData=[ L06137_dates, L08160_dates]
depthData=[depth_L06137, depth_L08160]
tgtYrs=[10,30,50,70]
lake_data2=[L06137_data, L08160_data]
#Define target years. Identify depths corresponding to target years.
iNd=0
dated_data_array=np.zeros([len(tgtYrs),dim_L1[1],len(ageData)])
...
lake_names=['Lake 06-137', 'Lake 08-160']
...
for z1 in np.arange(0,len(lake_data)):
    ...
    for z2 in np.arange(0,len(lake_data)):
        ...
        pToUse=np.linspace(0,3,4, dtype=int)
        paramArray=[]
        paramArray=np.array(paramArray, dtype=int)
        
        for t in np.linspace(0,3,4, dtype=int):
            varWiseAcc=[]
            iNd=0
            for k in np.linspace(1,len(pToUse),len(pToUse), dtype=int): #np.arange(1,batches):
                ...
                for j in np.arange(1,100):
                    ...
                    for i in range(0,batches-1):
                        ...
                        #Select test-sets. Will be reomved later from main dataset. I find that this is the most efficient way to do this
                        ...
                        clf = svm.SVC(kernel='rbf', C=3, class_weight='balanced') #Instantiate an SVM classifier. RBF is the default "I dunno what's going to happen" kernel
        ...
        ax1.plot(varWiseAcc2)
        #plt.plot(meanAcc2)
        ax1.set_ylabel('Accuracy')
        sTr="%s vs %s" % (lake_names[z1], lake_names[z2])
        ax1.set_xlabel(sTr)
        plt.draw()
        plt.pause(1)
